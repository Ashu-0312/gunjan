                            ...
                            if (response.isSuccessful) {
                                if (response.body()!!.code == 1) {
                                    FCSharedPreferances.getSharedPreferance(this@SettingsActivity).statuS_LOGIN =
                                        "false"
                                    FCSharedPreferances.getSharedPreferance(this@SettingsActivity).tokeN_STATUS =
                                        "true"
                                ...
                                if (response.isSuccessful) {
                                    if (response.body()!!.code == 1) {
                                        FCSharedPreferances.getSharedPreferance(this@SettingsActivity).savE_LANG =
                                            "en"
                                        val intent =
                                            Intent(this@SettingsActivity, HomeActivity::class.java)
                                        intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_CLEAR_TASK)
                                        startActivity(intent)
                                    } else {
                                        ProjectUtill.printMessage(
            ...
            object : Callback<AddCoinResponse> {
                override fun onResponse(
                    ...
                    if (response != null) {
                        if (response.isSuccessful) {
                            if (response.body()!!.code == 1) {
                                FCSharedPreferances.getSharedPreferance(this@SettingsActivity).totaL_COINS =
